...
The basic flooding algorithm is very simple since it requires switching nodes to maintain no state, besides the set of their active interfaces. Whenever there is a packet to forward that is not directed to the receiving node, the algorithm simply sends a copy of that packet to all the node interfaces but the one from which the packet arrived.
...
![The network used for testing the different versions of the algorithm](Figures/config4.1.png)
